...
const HeroSection = () => {
  // State for Video and Calculator Modal (No changes here)
  const [videoError, setVideoError] = useState(false);
  const [showCalculator, setShowCalculator] = useState(false);
  const [consumption, setConsumption] = useState<number>(0);
  const [stateValue, setStateValue] = useState<string>("Delhi");
  ...
  // ❌ All quote form state and logic has been removed.
  // (e.g., formData, loading, agreed, handleSubmit, etc.)

  // useEffect for mobile-only auto-scrolling (No changes here)
  useEffect(() => {
  ...
  // Calculator logic (No changes here)
  const stateTariffs: Record<string, number> = {
    Delhi: 8,
    "Uttar Pradesh": 7.2,
    Maharashtra: 9,
  ...
  return (
    <section className="relative z-0 min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          {/* Left Side: Text Content, CTAs, Stats */}
          <div className="text-center lg:text-left">
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white leading-tight mb-6">
              Powering Your Future with{" "}
              <span className="bg-gradient-to-r from-solar-orange to-solar-gold bg-clip-text text-transparent">
          ...
          <div id="quote-form" className="scroll-mt-24">
            {/* ✅ The original form Card has been replaced by the imported component */}
            <HeroGetQuote />
          </div>
        </div>
      </div>
      ...
      {/* Calculator Modal (No changes here) */}
      {showCalculator && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-lg relative">
            <button
            ...
            {results && (
              <div className="mt-6 bg-gray-50 p-4 rounded-lg">
                ...
                <div className="space-y-3">
                  <p className="flex items-center gap-2"><Zap className="text-yellow-500" /><strong>Required Solar System:</strong> <CountUp end={results.requiredKW} duration={2} /> kW</p>
                  <p className="flex items-center gap-2"><Zap className="text-green-600" /><strong>Monthly Generation:</strong> <CountUp end={results.monthlyGeneration} duration={2} /> units</p>
                  <p className="flex items-center gap-2"><Zap className="text-blue-500" /><strong>Yearly Generation:</strong> <CountUp end={results.yearlyGeneration} duration={2} /> units</p>
                  <p className="flex items-center gap-2"><Leaf className="text-green-700" /><strong>CO₂ Savings:</strong> <CountUp end={results.co2Savings} decimals={1} duration={2} /> tons/year</p>
                  <p className="flex items-center gap-2"><IndianRupee className="text-solar-orange" /><strong>Gross Cost:</strong> ₹<CountUp end={results.grossCost} duration={2} separator="," /></p>
                  <p className="flex items-center gap-2"><PiggyBank className="text-pink-500" /><strong>Government Subsidy:</strong> ₹<CountUp end={subsidy} duration={2} separator="," /></p>
                  <p className="flex items-center gap-2"><IndianRupee className="text-green-600" /><strong>Net Cost after Subsidy:</strong> ₹<CountUp end={results.netCost} duration={2} separator="," /></p>
                  <p className="flex items-center gap-2"><PiggyBank className="text-blue-600" /><strong>Monthly Savings:</strong> ₹<CountUp end={results.monthlySavings} duration={2} separator="," /></p>
                  <p className="flex items-center gap-2"><Timer className="text-gray-700" /><strong>Estimated Payback:</strong> <CountUp end={results.paybackMonths} duration={2} /> months (~<CountUp end={parseFloat(results.paybackYears)} decimals={1} duration={2} /> years)</p>
                </div>
              </div>
            )}
          </div>
